...
#_
(remove #(-> % :config :com.wsscode.pathom3.connect.operation/op-name
             name (= "resolver-for-stacktrace"))
        @global-resolvers)
...
(defn eql-from-state [editor-state]
  (let [resolver #(resolvers-from-state editor-state)
    ...
    (reset! global-eql (duck/gen-eql (remove #(-> % :config :com.wsscode.pathom3.connect.operation/op-name
                                                  name (= "resolver-for-stacktrace"))
                                             @global-resolvers)))
    (fn eql
...
; (defn eql
;   ([editor-state val] (eql editor-state {} val))
;   ([editor-state seed val]
;    (p/let [res (resolvers-from-state (:editor-state editor-state) :foo)]
;     (prn :RESULVERS-FROM-STATE res))))
;    ; (duck/eql seed val)))
